...
ret
```
Вместо HLT будем использовать *ret*, так как исполняемый буффер вызывается как функция с помощью *call*.
...
## Исполнение буффера, mprotect.

Теперь байт-код можно транслировать в IR, который позволит создать буффер закодированных иинструкций x86. Буффер виртуальной памяти даёт возможность выделить функция `mmap` из библиотеки *<sys/mman.h>*. Подробнее об использовании и возможностях этой функции можно прочитать по ссылкам в источники в соответствующем параграфе. 

К прочему, что буффер должен иметь права на исполнение, которые позволяет выдать функция `mprotect`.

Исполнить буффер можно с помощью ассемблерной вставки
...
В данной работе были поверхностно разобраны понятия JIT-компиляции, байт-кода и IR, а также система кодирования инструкций x86. Была реализована JIT-компиляция байт-кода в исполняемый буффер.
